...
mass_list = [round(0.1, 1)] + [round(0.12 + x * 0.020, 2)
                                            for x in range(95)]
mass_list = mass_list + [round(2.05 + x * 0.05, 2)
                                           for x in range(20)]
...
def convertO(x):
    ...
    if (x%1==0.0):
         ...
         return x
mass_list=list(map(convertO, mass_list))
period_list = ['0', '0.2', '0.4', '0.6', '0.8', '1', '1.2', '1.4', '1.6','1.8', '2', '2.2', '2.4', '2.6', '2.8', '3', '3.2', '3.4', '3.6', '3.8', '4']
combos=[]

for y in period_list:
      for z in possible_secondary_q:
            combos.append((z, y))
def assign_propsO(dicionario, input_str, y):
...
def find_mergersO(star_table, initial_mass):
    """
    ...
    If the mass of the primary star ends up being greater than the initial mass at the very first
    age represented by the star_table, then 0 is designated as the index where the merger occurs.
    If the merger does not occur then the length of the star_table is returned. (cannot be a valid index
...
def reformatterO(destination, metallicity):
    ...
    # have a good HashCode, adding to the HashSet should be fast in normal case.
    ...
    for x in metallicity:
        ...
        # Get a set of all Quasi-Chemicall Homogeneous Evolution (Binary) Models.
        d = set(glob.glob("{}/NEWSINMODS/{}hmg/*".format(hoki.MODELS_PATH, x)))
        # Get a set of all regular binary and merged binary models.
        b = set(glob.glob("{}/NEWBINMODS/NEWBINMODS/{}/*".format(hoki.MODELS_PATH, x)))
        # Get a set of all models where the primary is a compact remnant.
        c = set(glob.glob("{}/NEWBINMODS/NEWSECMODS/{}_2/*".format(hoki.MODELS_PATH, x)))
    ...
    new_bin_and_met=len(hoki.MODELS_PATH) + len("/NEBINMODS/NEWBINMODS/zxxx/")
    new_sec_and_met=len(hoki.MODELS_PATH) + len("/NEBINMODS/NEWBINMODS/zxxx_2/")
    for x in setofAll:
        ...
        # All columns (except column number 12, 13, 36) up to 48 inclusive will be kept
        ...
        if (x[len(hoki.MODELS_PATH)+1:len(hoki.MODELS_PATH)+1+len("NEWSINMODS")]=="NEWSINMODS" and x[new_sin_to_met+4:new_sin_to_met+7]!="hmg"):
            ...
            if x[len(hoki.MODELS_PATH)+1:len(hoki.MODELS_PATH)+1+len("NEWBINMODS/XXXXXXXXXX")]=="NEWBINMODS/NEWSECMODS":
                ...
                astroTable.write(destination + '/' + x[new_bin_to_met: new_bin_and_met]+
                                 "/FitsModels{}bin.fits".
                                 format(x[new_bin_and_met+1: ]),
                                 format='fits',overwrite = True)
                

def extractorO(age, metallicity, input_dir, bpass_evo_dir,
              margin):
    ...
    bpass_evo_dir: string
          ...
          preprocessed isochrone files for BPASS, will be stored in subdirectories
    ...
    Output:
    Saves into the specified destination directory a reformatted, FITS format version
    of a preprocessed BPASS isochrone of the specified age and metallicity whose individual
    rows correspond to stars of age closest to the specified age and within the specified
    ...
    for x in mass_list:
        ...
        [assign_propsO(names_to_prop, name, (x,)) for name in sec_files]
            ...
            if (len(f) != 0):
                ...
                if len(f) != 0:
                    ...
                    if (x[-8:-5] =='bin'):
                        ...
                        merge_pt = find_mergersO(org[['age','M1']], initlMass)
    ...
    if not isinstance(bigOne, type(None)) and not (bigOne['age'].empty):
        ...
        reduced.write("{}iso/{}/iso{}.fits".format(bpass_evo_dir, metallicity, str(age)), format = 'fits', overwrite = True)