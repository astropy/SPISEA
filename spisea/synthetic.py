...
class Binary_Cluster(Cluster):
    ...
    def make_singles_systems_table(self, isMulti, sysMass):
        ...
        star_systems.add_column( Column(self.iso.metallicity, name ='metallicity') )
        ...
        if self.ifmr != None:
            ...
            if 'metallicity_array' in inspect.getfullargspec(self.ifmr.generate_death_mass).args:
                r_mass_tmp, r_id_tmp = self.ifmr.generate_death_mass(mass_array = \
                                                                     star_systems['mass'][cdx_rem],
    ...
    def make_primaries_and_companions(self, star_systems, compMass):
        ...
        companions.add_column( Column(np.empty(N_comp_tot, dtype = bool), name = 'the_secondary_star?') )
        ...
        if self.ifmr != None:
            ...
            if 'metallicity_array' in inspect.getfullargspec(self.ifmr.generate_death_mass).args:
                r_mass_tmp, r_id_tmp = self.ifmr.generate_death_mass(mass_array = 
                                                                     star_systemsPrime['mass'][cdx_rem],
                                                                     metallicity_array = star_systemsPrime['metallicity'][cdx_rem])
                r_mass_tmp_c, r_id_tmp_c = self.ifmr.generate_death_mass(mass_array = 
                                                                         companions['mass'][cdx_rem_c],
                                                                     metallicity_array =
                                                                         companions['metallicity'][cdx_rem_c])
            else:
        ...
        # The compact remnants have photometric fluxes of nan now. So now we can procede
        # with the photometry.
